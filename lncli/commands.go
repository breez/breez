...
func saveRespJSON(resp proto.Message, filename string) {
	...
	if err != nil {
		...
		setResult("unable to decode response: " + err.Error())
		return
	}
	...
	if err != nil {
		...
		setResult(fmt.Sprintf("Error creating directory: %v: %v", directory, err))
		return
	}
	err = ioutil.WriteFile(filename, jsonOut.Bytes(), 0666)
	if err != nil {
		fmt.Printf("Error in WriteFile %v %v", filename, err)
		setResult(fmt.Sprintf("Error writing file: %v: %v", filename, err))
		return
	}

	setResult(fmt.Sprintf("Graph written to %s", filename))
}
...
func sendPayment(ctx *cli.Context) error {
	...
	if ctx.Bool("keysend") {
		...
		req.DestFeatures = []lnrpc.FeatureBit{
