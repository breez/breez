...
func (a *Service) FinishLNURLPay(params *data.LNURLPayResponse1) (*data.LNUrlPayInfo, error) {
	...
	*/
	...
	// FINDOUT Are these routes different from the ones in the invoice? If yes, why?
	// FINDOUT If these routes can be different, how do I use them in the client?
	/*
	   if len(payResponse2.Routes) > 0 {
	       for _, route := range payResponse2.Routes {
	           for _, hop := range r {

	               // ref. github.com/lightningnetwork/lnd/lnwire/channel_update.go
	               // ref. github.com/lightningnetwork/lnd/lnwire/message.go
	               // hop.ChannelUpdate string
	               // lnwire.ReadMessage validates a Lightning message.
	               _, err := lnwire.ReadMessage(strings.NewReader(hop.ChannelUpdate), 0) // TODO What should version number be?
	               if err != nil {
	                   return nil, fmt.Errorf("Unable to parse ChannelUpdate message for NodeId %s.", h.NodeId)
	               }
