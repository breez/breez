...
func shrinkInvoice(bolt11 string) (shrunkPayload []byte , err error) {
	hrp, data, err := decodeBech32(bolt11)
	if err != nil {
		return nil, err
	}

	smaller, err := bech32.ConvertBits(data, 5, 8, true)
	if err != nil {
		return nil, err
	}

	hrpLen := make([]byte, 1)
	hrpLen[0] = byte(len(hrp)-2)

	// Get rid of the 'ln' prefix and convert to bytes
	hrpBytes := append(hrpLen, []byte(hrp[2:])...)

	return append(hrpBytes, smaller...), nil
}

func ExpandInvoice(payload []byte) (bolt11 string, err error) {
	// extract hrp first
	var hrpDividerIndex = int(payload[0]) + 1

	hrp := "ln" + string(payload[1:hrpDividerIndex])

	data, err := bech32.ConvertBits(payload[hrpDividerIndex:], 8,5, false)
	if err != nil {
		return "", err
	}
...
// Following code pasted from LND's bech32.go
