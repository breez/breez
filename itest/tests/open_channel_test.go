...
func generateInvoiceWithNewAmount(payReq, nodeKey string, signerClient signrpc.SignerClient) (string, []byte, error) {
	invoice, err := zpay32.Decode(payReq, &chaincfg.SimNetParams)
	if err != nil {
	...
	invoice.Features = lnwire.NewFeatureVector(lnwire.NewRawFeatureVector(), make(map[lnwire.FeatureBit]string, 0))
